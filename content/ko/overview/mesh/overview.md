...
title: "개요"
weight: 1
description: "서비스 메시를 위한 Gateway API 사용"
---

{{< channel-version channel="standard" version="v1.1.0" >}}
서비스 메시 사용 사례를 지원하기 위한 [GAMMA 이니셔티브]({{< ref "/overview/mesh/gamma" >}}) 작업은
v1.1.0부터 표준 채널에 포함되었으며 GA로 간주된다.
자세한 정보는 [버전 관리 가이드]({{< ref "/overview/concepts/versioning" >}})를 참조하라.
{{< /channel-version >}}

"[GAMMA 이니셔티브]({{< ref "/overview/mesh/gamma" >}})"는 게이트웨이 API가 서비스 메시에서
...
## 라우트와 서비스 연결 {#gateway-api-for-mesh}
...
- 동일한 네임스페이스 {#producer-routes}

    서비스와 동일한 네임스페이스에 있는 라우트를 [프로듀서 라우트]라고 하는데,
    이는 일반적으로 워크로드의 생성자가 워크로드의 허용 가능한 사용을 정의하기 위해 생성하기 때문이다
...
[프로듀서 라우트]: {{< ref "/overview/glossary#producer-route" >}}
[컨슈머 라우트]: {{< ref "/overview/glossary#consumer-route" >}}
[서비스 메시]: {{< ref "/overview/glossary#service-mesh" >}}
[라우트 병합 규칙]: {{< ref "/reference/api-types/httproute#merging" >}}
