...
[커뮤니티]({{< ref "/contributing" >}}) 페이지에서 프로젝트에 참여하는 방법을
확인할 수 있다.
...
자세한 정보는 [인그레스에서 마이그레이션]({{< ref "/guides/getting-started/migrating-from-ingress" >}})
가이드를 참조하자.
...
* [정책 연결]({{< ref "/reference/policy-attachment" >}}) 모델을 사용하면
...
* API 정의 확장 지점을 사용한다.
  ...
  [확장 지점]({{< ref "/overview/concepts/api-overview#extension-points" >}})이 있다.
...
자세한 정보는 [버전 관리]({{< ref "/overview/concepts/versioning" >}}) 문서를 참조하라.

## 어떤 쿠버네티스 버전이 지원되는가?
[지원 버전]({{< ref "/overview/concepts/versioning#supported-versions" >}})에 대한 정책을
참조하자.
...
는
[TLSRoute]({{< ref "/overview/concepts/api-overview#tlsroute" >}})에서 지원된다.
패스스루 및 기타 TLS 구성에 대한 자세한 내용은
[TLS 가이드]({{< ref "/guides/user-guides/tls" >}})를 참조하자.
