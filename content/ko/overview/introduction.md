---
title: "소개"
description: "Kubernetes Gateway API - 쿠버네티스의 L4 및 L7 라우팅"
weight: 1
type: docs
---
...
전체 리소스 모델은 3개의 개별
[페르소나]({{< ref "/overview/concepts/roles-and-personas" >}})와
...
이러한 프로토콜별 리소스
([HTTPRoute]({{< ref "/reference/api-types/httproute" >}}), [GRPCRoute]({{< ref "/reference/api-types/grpcroute" >}}) 등)는
...
[북/남 트래픽]:/overview/glossary/#northsouth-traffic
[동/서 트래픽]:/overview/glossary/#eastwest-traffic
...
[게이트웨이]({{< ref "/reference/api-types/gateway" >}})와 [게이트웨이클래스]({{< ref "/reference/api-types/gatewayclass" >}}) 리소스는
사용되지 않는다.
대신, 개별 라우팅 리소스(예: [HTTPRoute]({{< ref "/reference/api-types/httproute" >}}))는
[서비스 리소스와 직접 연결][메시-연결]되며, 이를 통해 메시가 해당 서비스로 향하는
모든 트래픽을 관리하면서
게이트웨이 API의 역할 지향적 특성을 유지한다.

현재까지 [GAMMA]({{< ref "/overview/mesh" >}})는 게이트웨이 API에
최소한의 변경만으로 메시 기능을 지원해왔다.
하지만 GAMMA에서 특히 빠르게 중요해진 영역은
[서비스 리소스의 다양한 측면][서비스-측면] 정의이다.

[gamma]:/overview/mesh/
[서비스-메시]:/overview/glossary/#service-mesh
[서비스-측면]:/overview/mesh/service-facets/
[메시-연결]:/overview/mesh/
...
- [API 개요]({{< ref "/overview/concepts/api-overview" >}})
- [사용자 가이드]({{< ref "/guides/getting-started" >}})
- 구현
- [API 참조 사양]({{< ref "/reference/spec" >}})
- 커뮤니티 링크와 개발자 가이드
...
(이 세 가지에 대한 자세한 내용은 [역할과 페르소나]({{< ref "/overview/concepts/roles-and-personas" >}})에서
자세히 설명한다).
...
[사용-사례]:{{< ref "/overview/concepts/use-cases" >}}
...
프로젝트로 개발되고 있다.
최신 게이트웨이를 지원하는 프로젝트 및 제품은 구현 참조에서 확인할 수 있다.
