...
title: "사용 사례"
weight: 70
description: "Gateway API가 남북 및 동서 트래픽 시나리오를 처리하는 방법을 보여주는 예제 사용 사례"
---
...
[Ana]: {{< ref "/overview/concepts/roles-and-personas#key-roles-and-personas" >}}
[Chihiro]: {{< ref "/overview/concepts/roles-and-personas#key-roles-and-personas" >}}
[Ian]: {{< ref "/overview/concepts/roles-and-personas#key-roles-and-personas" >}}

## 사용 사례 목록

- [사용 사례](#use-cases)
  - [사용 사례 목록](#the-use-cases)
  - [기본 북/남(North/South) 사용 사례](#basic-northsouth-use-case)
  - [단일 게이트웨이 뒤에 여러 애플리케이션](#multiple-applications-behind-a-single-gateway)
  - [기본 동/서(East/West) 사용 사례](#basic-eastwest-use-case)
  - [게이트웨이와 메시 결합 사용 사례](#gateway-and-mesh-use-case)
...
릴리스 채널에 대한 자세한 내용은
[버전 관리 가이드]({{< ref "/overview/concepts/versioning" >}})를 참고하자.
{{< /channel-version >}}
...
이것은 Chihiro가 게이트웨이에서 [TLS]({{< ref "/guides/user-guides/tls#downstream-tls" >}})와
같은 중앙 정책을 강제할 수 있게 하며,
동시에 Ana와 동료들은 애플리케이션의 [라우팅 로직]({{< ref "/guides/user-guides/http-routing" >}})과
롤아웃 계획(예: [트래픽 분할 롤아웃]({{< ref "/guides/user-guides/traffic-splitting" >}}))을
직접 제어할 수 있도록 허용한다.
...
이 사례는
[기본 북/남 사용 사례](#basic-northsouth-use-case)와 놀라울 정도로 유사하지만,
여러 애플리케이션 팀이 존재한다.
...
이렇게 역할을 분리하면
Chihiro가 게이트웨이에서 [TLS]({{< ref "/guides/user-guides/tls#downstream-tls" >}})와 같은 중앙 정책을 강제할 수 있다.
한편, Ana와 Allison은 [각자의 네임스페이스]({{< ref "/guides/user-guides/multiple-ns" >}})에서 애플리케이션을 운영하면서
동일한 공유 게이트웨이에 라우트를 연결해,
[라우팅 로직]({{< ref "/guides/user-guides/http-routing" >}}),
[트래픽 분할 롤아웃]({{< ref "/guides/user-guides/traffic-splitting" >}}) 등을
독립적으로 제어할 수 있고,
Chihiro와 Ian이 처리하는 부분은 신경 쓰지 않아도 된다.

[HTTPRoute]: {{< ref "/reference/api-types/httproute" >}}
[게이트웨이 클래스]: {{< ref "/reference/api-types/gatewayclass" >}}
[게이트웨이]: {{< ref "/reference/api-types/gateway" >}}
[라우트 연결 프로세스]: {{< ref "/overview/concepts/api-overview#attaching-routes-to-gateways" >}}
...
[동/서]: {{< ref "/overview/glossary#eastwest-traffic" >}}
...
이는 사실상
[단일 게이트웨이 뒤에 여러 애플리케이션이 있는 경우](#multiple-applications-behind-a-single-gateway)와 [기본 동/서 사용 사례](#basic-eastwest-use-case)를
결합한 형태다.
...
Chihiro가 게이트웨이에서 [TLS]({{< ref "/guides/user-guides/tls#downstream-tls" >}})와 같은
중앙 정책을 강제할 수 있게 하면서,
Ana와 Allison이
[라우팅 로직]({{< ref "/guides/user-guides/http-routing" >}}), [트래픽 분할 롤아웃]({{< ref "/guides/user-guides/traffic-splitting" >}}) 등을
...
[게이트웨이 컨트롤러]: {{< ref "/overview/glossary#gateway-controller" >}}
[서비스 라우팅]: {{< ref "/overview/glossary#service-routing" >}}
[엔드포인트 라우팅]: {{< ref "/overview/glossary#endpoint-routing" >}}
