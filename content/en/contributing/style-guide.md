...
## Callouts
To implement callouts in documentation, we make use of Hugo shortcodes
such as `{{</* note */>}}` and `{{</* warning */>}}`.
...
channel. To accomplish that, we use the `channel-version` shortcode:
...
our [versioning guide]({{< ref "/overview/concepts/versioning" >}}).
{{< /channel-version >}}
