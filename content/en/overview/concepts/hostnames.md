...
title: "Hostnames"
weight: 100
description: "How Gateway API uses hostnames for route attachment, traffic discrimination, and routing."
---
...
In that case, the hostname is effectively the special value `*` - although this value is _not_ literally valid for the field, it's a convenient way to represent the "match anything" behavior for this document.
...
Also note that RFC-2818 has the wildcard character `*` only match a _single_ DNS label, rather than multiple. So the SNI behavior _may be_ subtly different to the hostname intersection and Listener selection behavior, when the TLS connection is actually _terminated_. Many Gateway API implementations have proxy data planes that match the SNI as a suffix match, however, and in that case the matching behavior will be the same.
...
| `www.example.com` | `foo.example.com` | No |
| `*.example.com` | `www.example.com` | Yes |
| `*.example.com` | `foo.example.com` | Yes |
| `*.example.com` | `foo.bar.example.com` | Yes* |
...
|---|---|---|---|---|---|---|---|---|---|---|
|`www.example.com` | None | HTTPRoute | `www.example.com` | Yes | `www.example.com` | | | `www.example.com` | Yes ||
|`*.example.com` | None | HTTPRoute | `www.example.com` | Yes | `www.example.com` | | | `www.example.com` | Yes ||
|`*.example.com` | None | HTTPRoute | `*.com` | Yes | `*.example.com` | | | `www.example.com` | Yes ||
|`*.example.com` | None | HTTPRoute | `*.com` | Yes | `*.example.com` | | | `foo.bar.example.com` | Yes | Wildcard matches for Host header will match one _or more_ DNS labels.|
|`*.example.com` | None | HTTPRoute | `www.example.com` | Yes | `www.example.com` | | | `example.com` | No ||
|`*.example.com` | None | HTTPRoute | `www.example.com` | Yes | `www.example.com` | | | `foo.example.com` | No ||
| `*.example.com` | Terminated | HTTPRoute | `www.example.com` | Yes | `www.example.com` |`www.example.com` | Yes | `www.example.com` | Yes ||
| `*.example.com` | Terminated | HTTPRoute | `foo.bar.example.com` | Yes | `foo.bar.example.com` |`foo.bar.example.com` | Yes | `foo.bar.example.com` | Yes | SSL Certificate _must_ match **intersected hostname**, not Listener hostname, or else SNI matching will fail, because `*.example.com` on a certificate does _not_ match `foo.bar.example.com` as an SNI. |
| `*.example.com` | Terminated | HTTPRoute | `*.example.com` | Yes | `*.example.com` |`foo.bar.example.com` | No | || `*.example.com` on a certificate does _not_ match `foo.bar.example.com` as an SNI. |
| `*.example.com` | Terminated | HTTPRoute | `foo.example.com` | Yes | `foo.example.com` |`foo.example.com` | Yes | `foo.example.com` | Yes ||
| `www.example.com` | Passthrough | TLSRoute | `www.example.com` | Yes | `www.example.com` | `www.example.com`  | Yes | |||
| `*.example.com` | Passthrough | TLSRoute | `www.example.com` | Yes | `www.example.com` | `www.example.com`  | Yes | |||
| `*.example.com` | Passthrough | TLSRoute | `www.example.com` | Yes | `www.example.com` | `foo.example.com`  |  No  | |||
| `*.example.com` | Passthrough | TLSRoute | `foo.bar.example.com` | Yes | `foo.bar.example.com` | `www.example.com`  | No | ||The SNI must match the intersected hostname.|
| `*.example.com` | Passthrough | TLSRoute | unset | Yes | `*.example.com` | `www.example.com`  | Yes | |||
| `*.example.com` | Passthrough | TLSRoute | unset | Yes | `*.example.com` | `foo.bar.example.com`  | No | ||SNI matches against wildcard names in certificates can _only_ match a single DNS label. (This assumes that the certificate name matches the intersected hostname, which is not required.)|
| `www.example.com` | Terminated | TLSRoute | `www.example.com` | Yes | `www.example.com` | `www.example.com`  | Yes | ||The examples for TLSRoute in Terminated mode are the same as the examples for TLSRoute in Passthrough mode and have been elided.|
...
Gateway API is working on a guide on how the API's design intends for wildcard certificates to be managed, but the general approach we expect is that,
because wildcard certificates have a _significantly_ larger security impact, they should not be managed automatically without using a dedicated API for that.

So, the cert-manager Certificate API is fine - as that is designed around managing wildcard certificates, and when combined with cross-namespace TLS references
on a Listener and ReferenceGrants, can be used in a real cluster relatively safely (almost certainly meeting the OWASP "be careful with wildcard certificates" bar).
