...
title: "Security"
weight: 50
description: "Security model for Gateway API including RBAC, cross-namespace references, and best practices."
---
...
The [hostname definition]({{< ref "/reference/spec#httproutespec" >}}) on a Route or on ListenerSet is a list, and given the following scenario:
...
```
{{< /tab >}}
{{< tab name="Insecure configuration" >}}
```yaml
apiVersion: gateway.networking.k8s.io/v1
kind: Gateway
metadata:
...
A [ValidatingAdmissionPolicy](https://kubernetes.io/reference/access-authn-authz/validating-admission-policy/)
...
Owners of resources should be aware of the usage of [ReferenceGrants]({{< ref "/reference/api-types/referencegrant" >}}).
...
be aware of it and create [Roles](https://kubernetes.io/reference/access-authn-authz/rbac/#role-and-clusterrole)
and [RoleBindings](https://kubernetes.io/reference/access-authn-authz/rbac/#rolebinding-and-clusterrolebinding)
