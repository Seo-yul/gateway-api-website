...
[HTTPRoute resources]({{< ref "/reference/api-types/httproute" >}}) can modify the headers of HTTP requests and the HTTP responses from clients.
There are two types of [filters]({{< ref "/reference/api-types/httproute#filters-optional" >}}) available to meet these requirements: `RequestHeaderModifier` and `ResponseHeaderModifier`.

This guide shows how to use these features.

Note that these features are compatible. HTTP headers of the incoming requests and the headers of their responses can both be modified using a single [HTTPRoute resource]({{< ref "/reference/api-types/httproute" >}}).
