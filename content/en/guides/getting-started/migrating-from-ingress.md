...
Gateway API is the successor to the [Ingress API][ing]. This guide provides a general overview for migrating from any Ingress implementation to Gateway API. It focuses on the core concepts, differences, and feature mappings between Ingress and Gateway API, and provides a manual conversion example.

[ing]:https://kubernetes.io/overview/concepts/services-networking/ingress/

{{< note >}}
If you are specifically using Ingress-NGINX, we recommend also checking out the [Ingress-NGINX Migration Guide]({{< ref "/guides/getting-started/migrating-from-ingress-nginx" >}}) for more tailored advice and resources.
{{< /note >}}
...
The [Ingress API](https://kubernetes.io/overview/concepts/services-networking/ingress/)
...
vendors with many implementations ([Ingress controllers](https://kubernetes.io/overview/concepts/services-networking/ingress-controllers/))
...
> Read more about the [design goals]({{< ref "/overview#gateway-api-concepts" >}})
> of Gateway API.
...
the [IngressClass](https://kubernetes.io/overview/concepts/services-networking/ingress/#ingress-class)
...
Gateway API
includes [four explicit personas]({{< ref "/overview/concepts/security#roles-and-personas" >}}):
...
[anns]:https://kubernetes.io/overview/concepts/overview/working-with-objects/annotations/
...
*   [Resource backends](https://kubernetes.io/overview/concepts/services-networking/ingress/#resource-backend),
...
*   *External references.* A feature (field) of a Gateway API resource can
    ...
    *   [HTTPRouteFilter]({{< ref "/reference/spec#httproutefilter" >}})
        can reference an external resource via the `extensionRef` field, thus
        configuring an implementation-specific filter.
    *   [BackendObjectReference]({{< ref "/reference/spec#backendobjectreference" >}})
        supports resources other than Services.
    *   [SecretObjectReference]({{< ref "/reference/spec#secretobjectreference" >}})
...
*   *Custom implementations*. For some features, it is left up to an
    ...
    (custom)  [conformance level]({{< ref "/overview/concepts/conformance#2-support-levels" >}}). For
    example:
    *   The `RegularExpression` type of
        the [HTTPPathMatch]({{< ref "/reference/spec#httppathmatch" >}}).
*   *Policies.* A Gateway implementation can define custom resources called
    ...
    standard UX. See the [Policy attachment guide]({{< ref "/reference/policy-attachment" >}})
...
a [Gateway]({{< ref "/reference/api-types/gateway" >}}) resource. For example, if you want the data
plane to handle HTTP traffic on port 80, you need to define
a [listener]({{< ref "/reference/spec#listener" >}}) for
...
the [TLS section](https://kubernetes.io/overview/concepts/services-networking/ingress/#tls),
where the TLS certificate and key are stored in a Secret.

In Gateway API, TLS termination is a property of
the [Gateway listener]({{< ref "/reference/spec#listener" >}}),
...
The [path-based routing rules](https://kubernetes.io/overview/concepts/services-networking/ingress/#path-types)
of the Ingress resource map directly to
the [routing rules]({{< ref "/reference/spec#httprouterule" >}})
of the [HTTPRoute]({{< ref "/reference/api-types/httproute" >}}).

The [host-header-based routing rules](https://kubernetes.io/overview/concepts/services-networking/ingress/#name-based-virtual-hosting)
map to
the [hostnames]({{< ref "/reference/spec#hostname" >}}) of
...
> The `hostnames` of an HTTPRoute must match the `hostname` of the [Gateway listener]({{< ref "/reference/spec#listener" >}}).
> Otherwise, the listener will ignore the routing rules for the unmatched
> hostnames. See the [HTTPRoute documentation]({{< ref "/reference/spec#httproutespec" >}}).
...
*   Conflicts must be handled as
    prescribed in [API Design Guide: Conflicts]({{< ref "/guides/api-design#conflicts" >}}). For example, more specific
    matches in a routing rule win over the less specific ones.

#### Default Backend

The
Ingress [default backend](https://kubernetes.io/overview/concepts/services-networking/ingress/#default-backend)
...
a [class](https://kubernetes.io/overview/concepts/services-networking/ingress/#ingress-class)
to select which Ingress controller to use. An HTTPRoute must specify which
Gateway (or Gateways) to attach to via
a [parentRef]({{< ref "/reference/spec#parentreference" >}}).
