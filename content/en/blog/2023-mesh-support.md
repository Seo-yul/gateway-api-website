...
date: 2023-08-29
description: "We are excited to announce the v0.8.0 release of Gateway API, where service mesh support has now reached Experimental status, we've introduced CEL validation and a Mesh conformance profile, and more!"
type: docs
---
...
status](/geps/overview/#gep-states), and we've also made some smaller additions such as CEL
...
2022, the Gateway API subproject started the [GAMMA initiative](/overview/mesh/), a
...
small number of [enhancement proposals](/contributing/enhancement-requests/) that subsume many hours of
...
documentation](/overview/mesh/) and [GEP-1426](/geps/gep-1294/), but the short version for Gateway
...
restarting on Gateway API _conformance profiles_, as discussed in [GEP-1709](/geps/gep-1709/).
...
[GEP-1686](/geps/gep-1686/), which checks only the mesh functionality as defined by GAMMA. At
...
from webhook validation to [CEL validation](https://kubernetes.io/docs/reference/using-api/cel/) using information built into
...
Kubernetes](/overview/concepts/versioning/#supported-versions). As such, these versions are no longer
...
necessity) taken a hard look at the process around [Experimental](/geps/overview/#gep-states)
...
the GEP process such that reaching [Experimental](/geps/overview/#gep-states) _requires_ that a
GEP include both the graduation criteria by which the GEP will become
[Standard](/geps/overview/#gep-states), and a probationary period after which the GEP will be
...
the [v0.8.0 release notes](https://github.com/kubernetes-sigs/gateway-api/releases/tag/v0.8.0). For more information on Gateway API versioning,
refer to the [official documentation](/overview/concepts/versioning/).

## How can I get started with the Gateway API?

Gateway API represents the future of load balancing, routing, and service mesh
APIs in Kubernetes. There are already more than 20 [implementations](/overview/implementations/)
...
[API concepts documentation](/overview/concepts/api-overview/) and check out some of the
...
page](/contributing/) which includes links to the Slack channel and community
meetings. We look forward to seeing you!!

## Further Reading:

- [GEP-1324](/geps/gep-1324/) provides an overview of the GAMMA goals and some important
  definitions. This GEP is well worth a read for its discussion of the problem
  space.
- [GEP-1426](/geps/gep-1294/) defines how to use Gateway API route resources, such as
  HTTPRoute, to manage traffic within a service mesh.
- [GEP-1686](/geps/gep-1686/) builds on the work of [GEP-1709](/geps/gep-1709/) to define a _conformance
  profile_ for service meshes to be declared conformant with the Gateway API.

Although these are [Experimental](/geps/overview/#gep-states) patterns, note that they are
available in the [`standard` release channel](/overview/concepts/versioning/#release-channels), since the GAMMA initiative
