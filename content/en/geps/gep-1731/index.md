...
description: 'GEP-1731: HTTPRoute Retries (Experimental)'
weight: 1731
status: Experimental
authors:
- mikemorris
relationships:
  ...
  - number: 1742
    ...
    description: Covers some overlapping considerations around when requests should
      be retried.
featureNames:
...
* To define any interaction with configured HTTPRoute [timeouts]({{< ref "/geps/gep-1742" >}}).
...
Implementing a `requestRetryPolicy` [HTTPRouteFilter]({{< ref "/reference/spec" >}}#httproutefilter) type is likely a reasonable alternative implementation (with the downside of slightly deeping nesting and more complex structural configuration) that was not fully explored.

Adding a new field to HTTPRouteRule instead is proposed for parity with the similar and intersecting configuration of [HTTPRouteTimeouts]({{< ref "/reference/spec" >}}#httproutetimeouts).
