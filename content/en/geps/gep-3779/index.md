...
description: 'GEP-3779: Identity Based Authz for east-west traffic (Implementable)'
weight: 3779
...
(Using the [Gateway API Personas]({{< ref "/overview/concepts/roles-and-personas" >}}))
...
For the full spec and semantics of Istio AuthorizationPolicy: [Istio authorization policy docs](https://istio.io/latest/reference/config/security/authorization-policy/)

Istio's AuthorizationPolicy can enforce access control by specifying allowed istio-formatted identities using the `source.principals` field, which matches authenticated Serviceaccount identities via mTLS. You can also use other source constructs which are described in the table above and in https://istio.io/latest/reference/config/security/authorization-policy/#Source.
