...
description: 'GEP-1324: Service Mesh in Gateway API (Memorandum)'
weight: 1324
status: Memorandum
authors:
- keithmattix
...
[expprob]: {{< ref "/geps/overview" >}}#probationary-period
...
Features or other modifications to the Gateway API spec that fall under this GEP will be subject to the same [versioning guidelines]({{< ref "/overview/concepts/versioning" >}}#graduation-criteria) as the rest of the Gateway API. For example, to move changes concerning a beta feature (e.g. HTTPRoute) from experimental to standard, all of the [graduation criteria]({{< ref "/overview/concepts/versioning" >}}#graduation-criteria) must be met (e.g. implemented by several implementations).
...
Policy is a broad term for any resource that can attach to another resource to configure it. This is defined in the [Policy Attachment GEP]({{< ref "/geps/gep-713" >}}). Examples of (hypothetical) policies could be resources like `TimeoutPolicy` or `AuthorizationPolicy`.
