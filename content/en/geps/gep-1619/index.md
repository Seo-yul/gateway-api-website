...
description: 'GEP-1619: Session Persistence (Experimental)'
weight: 1619
status: Experimental
authors:
- gcs278
...
relationships:
  ...
  - number: 713
    ...
    description: Defines metaresource and policy attachment API
  - number: 2257
    name: Gateway API Duration Format
    description: Defines the Duration API field
---
...
(See [status definitions]({{< ref "/geps/overview" >}}#gep-states).)
...
| Istio | [Strong Session Affinity](https://istio.io/latest/reference/config/networking/destination-rule/#LoadBalancerSettings-ConsistentHashLB) | [Soft Session Affinity](https://istio.io/latest/reference/config/networking/destination-rule/#LoadBalancerSettings-ConsistentHashLB) |
...
implies session affinity can use cookies or headers, as seen in Istio's [ConsistentHashLB](https://istio.io/latest/reference/config/networking/destination-rule/#LoadBalancerSettings-ConsistentHashLB). With session
...
| Istio 	| Implementation (Envoy) 	| [Cookie-Based](https://istio.io/latest/reference/config/networking/destination-rule/#LoadBalancerSettings-ConsistentHashLB-HTTPCookie) 	| Name=name<br>Path=path<br>TTL=duration 	| [ConsistentHashLB](https://istio.io/latest/reference/config/networking/destination-rule/#LoadBalancerSettings-ConsistentHashLB) (Backends) 	| Istio also supports turning on cookie-based session persistence via the Pilot ENV variable [PILOT_PERSISTENT_SESSION_LABEL](https://istio.io/latest/reference/commands/pilot-discovery/#envvars). 	|
|  	| Implementation (Envoy) 	| [Header-Based](https://istio.io/latest/reference/config/networking/destination-rule/#LoadBalancerSettings-ConsistentHashLB) 	| Name=name 	| [ConsistentHashLB](https://istio.io/latest/reference/config/networking/destination-rule/#LoadBalancerSettings-ConsistentHashLB) (Backends) 	| N/A 	|
...
Kubernetes provides an API that allows you to enable [session affinity](https://kubernetes.io/reference/networking/virtual-ips/#session-affinity)
...
In order to apply session persistence configuration to a backend, we will implement it as a [Policy Attachment]({{< ref "/reference/policy-attachment" >}}).
The new metaresource is named `BackendLBPolicy` and is responsible for configuring load balancing-related configuration
for traffic intended for a backend after routing has occurred. It is defined as a [Direct Policy Attachment]({{< ref "/geps/gep-713" >}}#direct-policy-attachment)
...
Referring to the [Gateway API Security Model]({{< ref "/overview/concepts/security" >}}#roles-and-personas),
...
ServiceImport (see [GEP-1748]({{< ref "/geps/gep-1748" >}})), or any implementation-specific backends that are a valid
[`BackendObjectReference`]({{< ref "/reference/spec" >}}#backendobjectreference). Enabling session
persistence for a backend enables subsequently enables it for any route directing traffic to this backend. To learn more
about the process of attaching a policy to a backend, please refer to [GEP-713]({{< ref "/geps/gep-713" >}}).
...
In this situation, implementations MAY want to indicate a warning via a log or status. Refer to [GEP-713]({{< ref "/geps/gep-713" >}})
and/or [GEP-2648]({{< ref "/geps/gep-2648" >}}) for more specific details on how to handle override scenarios.
...
The object of the [GAMMA (Gateway API for Mesh Management and Administration)]({{< ref "/overview/mesh/gamma" >}})
initiative is to provide support for service mesh and mesh-adjacent use-cases with Gateway API. GAMMA is focused on
defining how Gateway API could also be used for inter-service or [east/west]({{< ref "/overview/glossary" >}}#eastwest-traffic)
...
In Kubernetes, it's possible to modify the [selector](https://kubernetes.io/overview/concepts/services-networking/service/#services-in-kubernetes)
...
configuration via a warning status or log.
    - This might require addressing as part of an update to [GEP-2648]({{< ref "/geps/gep-2648" >}}).
...
- Kubernetes defines [session affinity](https://kubernetes.io/reference/networking/virtual-ips/#session-affinity) based on client IP hashing (same as our session affinity)
...
- [Kube-Proxy Session Affinity](https://kubernetes.io/reference/networking/virtual-ips/#session-affinity)
- [GEP-713: Metaresources and PolicyAttachment]({{< ref "/geps/gep-713" >}})
- [RFC6265](https://www.rfc-editor.org/rfc/rfc6265)
- [Policy Attachment]({{< ref "/reference/policy-attachment" >}})
