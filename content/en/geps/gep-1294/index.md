...
description: 'GEP-1294: xRoutes Mesh Binding (Standard)'
weight: 1294
status: Standard
authors:
- mikemorris
...
This GEP uses the [roles and personas]({{< ref "/overview/concepts/security" >}}#roles-and-personas) defined in the Gateway API security model, and the service "producer" and "consumer" roles defined in [GEP-1324: Service Mesh in Gateway API]({{< ref "/geps/gep-1324" >}}#producer-and-consumer).
...
* Defining how [Policy Attachment]({{< ref "/reference/policy-attachment" >}}) would bind to `xRoute`, services or a mesh.
...
In the example above, routing rules have been configured to direct 90% of traffic for the `foo` `Service` to the default "backend" endpoints specified by the `foo` `Service` [`selector`](https://kubernetes.io/overview/concepts/services-networking/service/#defining-a-service) field, and 10% to the `foo-v2` `Service`. This is determined based on the `ClusterIP` (for `Service`) and `ClusterSetIP` (for `ServiceImport`) matching, and for "transparent proxy" mesh implementations would match all requests to `foo.svc.cluster.local` (or arbitrary custom suffix, as the hostname is not specified manually) from within the same namespace, all requests to `foo.store.svc.cluster.local` from other namespaces, and all requests to `foo.store.svc.clusterset.local` for multicluster services, within the scope of the service mesh.
...
Implementations SHOULD support the default [`ClusterIP`](https://kubernetes.io/overview/concepts/services-networking/service/#publishing-services-service-types) `Service` type as a `parentRef`, with or without selectors.

["Headless" `Services`](https://kubernetes.io/overview/concepts/services-networking/service/#headless-services) SHOULD NOT be supported as a `parentRef`, because they do not implement the "frontend" functionality of a service.

`Service` resource with [`type: NodePort`](https://kubernetes.io/overview/concepts/services-networking/service/#type-nodeport) or [`type: LoadBalancer`](https://kubernetes.io/overview/concepts/services-networking/service/#loadbalancer) MAY be allowed as `parentRefs` or `backendRefs`, as these do provision virtual IPs and are effectively `ClusterIP` services with additional functionality, but it should typically be preferred to expose services publicly through the North/South Gateway API interfaces instead.

`Service` resources with [`type: ExternalName`](https://kubernetes.io/overview/concepts/services-networking/service/#externalname) SHOULD NOT be allowed as `parentRefs` or `backendRefs` due to [security concerns](https://github.com/kubernetes/kubernetes/issues/103675), although might eventually play some role in [configuring egress functionality](https://github.com/kubernetes-sigs/gateway-api/issues/1070).
...
Implementations that support [GEP-1748: Gateway API Interaction with Multi-Cluster Services]({{< ref "/geps/gep-1748" >}}) MAY allow `ServiceImport` to be used as a `backendRef`. When supported, the semantics MUST follow GEP-1748. Headless `ServiceImport` is out of scope and not currently a part of the spec.
...
In addition to Service, there are other optional parentRef resources that, if used by implementations, MUST adhere to the spec’s prescriptions. At the time of writing (v0.7.0), there is one resource in extended conformance: `ServiceImport` (part of the [MCS API](https://github.com/kubernetes-sigs/mcs-api), currently in alpha). The semantics of `ServiceImport` `parentRef` binding can be found in [GEP-1748]({{< ref "/geps/gep-1748" >}}) (Note: Headless `ServiceImport` is out of scope and not currently a part of the spec).
...
For the use case of filtering incoming traffic from selected HTTP hostnames, it is recommended to guide users toward configuring [`HTTPHeaderMatch`]({{< ref "/reference/spec" >}}#httpheadermatch) rules for the [`Host`](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Host) header. Functionality to be explored in future GEPs may include supporting concurrent usage of an `xRoute` traffic configuration for multiple North/South `Gateways` and East/West mesh use cases or redirection of egress traffic to an in-cluster `Service`.
...
* [GEP-1282: Describing Backend Properties]({{< ref "/geps/gep-1282" >}}) may be one path to associating a `Service` with a mesh, but likely wouldn't be able to handle the application of multiple `HTTPRoutes` for the same `Service`, but each intended for different mesh implementations
...
Separate `HttpService`, `TlsService` and `TcpService` resources could have the benefit of allowing us to define protocol specific elements to the spec along with an embedded `CommonServiceSpec`, similar to [`CommonRouteSpec`]({{< ref "/reference/spec" >}}#commonroutespec), and keep similar patterns as `Service`.
...
It is currently undefined how this approach may interact with either explicitly configured [`hostnames`]({{< ref "/reference/spec" >}}#httproutespec) or implicit "transparent proxy" routing for Kubernetes `Services` to determine how traffic should be intercepted and redirected.
...
#### Nested `services` and `hostnames` fields in [`ParentReference`]({{< ref "/reference/spec" >}}#parentreference)
...
A new field `serviceBinding` would be added to `HTTPRoute` to attach to the `Service`. Alternatively, this could be a new field in [`HTTPRouteMatch`]({{< ref "/reference/spec" >}}#httproutematch). As with the proposed implementation, this approach could be combined with a `Mesh` resource or similar as the `parentRef`, which would just define that the route would be applied to a mesh.
...
* `ServiceProjection` could handle the full responsibilities described in [GEP-1282: Describing Backend Properties]({{< ref "/geps/gep-1282" >}})
